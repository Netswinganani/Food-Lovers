...
app.post("/signup", (req, res) => {
  ...
  if (name.length < 3) {
    ...
    return res.json({
      alert: "Password should be atleast eight characters long",
    });
...
app.post("/login", (req, res) => {
  ...
  if (!email.length || !password.length) {
    return res.json({ alert: "Kindly fill all the Inputs" });
  }
...
app.post("/add-product", (req, res) => {
  ...
  if (!name.length) {
    ...
    return res.json({
      alert: "Please enter Detailed Description about the Product",
    });
...
app.post("/get-seller", (req, res) => {
  ...
  docref.get().then((doc) => {
    if (doc.exists) {
      ...
      res.status(404).json({ alert: "Seller not found" });
    }
  }).catch((error) => {
    console.error("Error fetching seller data:", error);
    res.status(500).json({ alert: "Error fetching seller data" });
  });
});
...
app.post("/toggle-product", (req, res) => {
  const { data } = req.body;
            ...
            <html lang="en">
                ...
                <body>
                    <div class="mail">
                        ...
                        <p class="span">Regards, Foodlovers</p>
    ...
    .then((data) => {
      transporter.sendMail(mailOptions, (err, info) => {
        if (err) {
          ...
          res.json({
            alert:
              "Opps! It seems like there is some problem with our server. Please try again later.",
          });
        } else {
